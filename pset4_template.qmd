...
#fill NA values in ZIP_CD and PRVDR_NUM with 0 
Texas_hospitals["ZIP_CD"] = Texas_hospitals["ZIP_CD"].fillna(0)

Texas_hospitals["PRVDR_NUM"] = Texas_hospitals["PRVDR_NUM"].fillna(0)

#make choropleth
Texas_hospitals.plot(column = "PRVDR_NUM", legend = True)
```
...
```
    ...
    This value does make sense as all distance in the US is measured in miles, units is not useful to gauge real life distance.
...
```{python}
print(type(zips_texas_centroids))
...
# Generate the list of ZIP codes
Tex_zip_list = np.linspace(start=750, stop=799, num=50).astype(np.int32)

# Filter the GeoDataFrame to include only the relevant ZIP codes
filtered_zips = zips_texas_centroids[zips_texas_centroids['ZIP_C'].isin(Tex_zip_list)]


fig, ax = plt.subplots(1, 1, figsize=(12, 10))

filtered_zips.plot(column='Nearest_Hospital_Distance_Miles',
                   ax=ax,
                   legend=True,
                   cmap='OrRd',
                   edgecolor='black',
...
ax.set_title('Distance to Nearest Hospital for Selected Texas ZIP Codes', fontsize=15)
...
```{python}
shape_closure.centroid.buffer(1).plot(edgecolor="white").set_axis_off()
plt.axis("off")
plt.show()
